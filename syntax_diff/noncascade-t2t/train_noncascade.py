...
from diffusion.noncascade_diffusion import GaussianDiffusion, Trainer
...
def main(args):
    ...
    if args.eval:
        ...
        if args.class_conditional:
            for class_id in range(model.num_classes):
                trainer.sample_seq2seq(class_id=class_id, cls_free_guidance=2.0)
        else:
    ...
    if args.eval_test:
        ...
        if args.class_conditional:
            for class_id in range(model.num_classes):
                trainer.sample_seq2seq(class_id=class_id, test=True, cls_free_guidance=2.0)
        else:
...
if __name__ == "__main__":
    ...
    if args.eval or args.resume_training or args.eval_test:
        ...
        heldout_params = {'wandb_name', 'wandb_project', 'output_dir', 'resume_dir', 'eval', 'eval_test', 'num_samples', 'sampling_timesteps', 'sampling_schedule', 'scale', 'sampler', 'resume_training'}
