...
from diffusion.cascade_diffusion import GaussianDiffusion, Trainer
...
def main(args):
    ...
    diffusion = GaussianDiffusion(
        ...
        cascade_unconditional_prob = args.cascade_unconditional_prob,
    ).to(device)

    cascade_syntax_diffusion = None
    if args.cascade:
        cascade_syntax_model = DiffusionTransformer(
            tx_dim = args.tx_dim,
            tx_depth = args.tx_depth,
            heads = args.tx_dim//ATTN_HEAD_DIM,
            lm_dim = lm_dim,
        ...
        cascade_syntax_diffusion = GaussianDiffusion(
            cascade_syntax_model,
            max_seq_len = cascade_syntax_model.max_seq_len,
            sampling_timesteps = args.sampling_timesteps,     # number of sampling steps
            loss_type = args.loss_type,            # L1 or L2
            objective = args.objective,
            train_schedule= args.train_schedule, 
    ...
    trainer = Trainer(
        ...
        cascade_syntax_diffusion=cascade_syntax_diffusion,
        cascade_syntax_path=args.cascade_syntax_path,
    )
    ...
    if args.eval:
        ...
        if args.class_conditional:
            for class_id in range(model.num_classes):
                trainer.sample_cascade(class_id=class_id, cls_free_guidance=2.0)
        else:
            trainer.sample_cascade(cls_free_guidance=2.0)
        return
        return
    ...
    if args.cascade_gen:
        ...
        if args.class_conditional:
            for class_id in range(model.num_classes):
                trainer.sample_cascade(class_id=class_id, test=True, cls_free_guidance=2.0)
        else:
            trainer.sample_cascade(test=True, cls_free_guidance=2.0)
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cascade_unconditional_prob", type=float, default=0.1)
    # Load and eval model
    ...
    parser.add_argument("--cascade_syntax_path", type=str, default=None)
    parser.add_argument("--cascade_gen", action="store_true", default=False)
    ...
    if args.eval or args.resume_training or args.cascade_gen:
        with open(os.path.join(args.resume_dir, 'args.json'), 'rt') as f:
            saved_args = json.load(f)
        args_dict = vars(args)
        # Hold out sampling/evaluation parameters
        heldout_params = {'wandb_name', 'wandb_project', 'output_dir', 'cascade_syntax_path', 'resume_dir', 'eval', 'cascade_gen', 'num_samples', 'num_train_steps', 'sampling_timesteps', 'sampling_schedule', 'scale', 'sampler', 'resume_training'}
