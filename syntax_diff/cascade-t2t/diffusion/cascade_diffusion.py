...
class GaussianDiffusion(nn.Module):
    ...
    def diffusion_model_predictions(self, z_t, mask, t, *, x_self_cond = None, class_id=None, context=None, context_mask=None,
        ...
        model_output, attn_maps = self.diffusion_model(z_t, mask, time_cond, x_self_cond, class_id=class_id, context=context, context_mask=context_mask,
    ...
    @torch.no_grad()
    def ddpm_sample(self, shape, lengths, class_id, context, context_mask, cascade_cond, cascade_mask, cascade_time, cls_free_guidance=1.0, z_t=None):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling step', total = self.sampling_timesteps):
            # get predicted x0
            model_output, _ = self.diffusion_model_predictions(z_t, mask, time, class_id=class_id, x_self_cond=x_start, context=context, context_mask=context_mask, cascade_cond=cascade_cond, cascade_mask=cascade_mask, cascade_time=cascade_time, sampling=True, cls_free_guidance=cls_free_guidance)
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, lengths, class_id, context, context_mask, cascade_cond, cascade_mask, cascade_time, cls_free_guidance=1.0, z_t=None):
        batch_size, device = shape[0], next(self.diffusion_model.parameters()).device
        time_pairs = self.get_sampling_timesteps(batch_size, device = device)
    ...
    @torch.no_grad()
    def sample(self, batch_size, length, class_id=None, context=None, context_mask=None, cascade_cond=None, cascade_mask=None, cascade_time=None, cls_free_guidance=1.0):
        ...
        return sample_fn((batch_size, max_seq_len, lm_dim), length, class_id, context, context_mask, cascade_cond, cascade_mask, cascade_time, cls_free_guidance)
    ...
    def forward(self, z_0, mask, class_id, context, context_mask,
        ...
        if self.self_condition and (random.random() < self.train_prob_self_cond):
            with torch.no_grad():
                model_output,_ = self.diffusion_model_predictions(z_t, mask, times, class_id=class_id, context=context, context_mask=context_mask,
                                                                  cascade_cond=cascade_cond, cascade_mask=cascade_mask, cascade_time=cascade_time)
                self_cond = model_output.pred_x_start.detach()
        # predict and take gradient step
        predictions,_ = self.diffusion_model_predictions(z_t, mask, times, x_self_cond=self_cond, class_id=class_id, context=context, context_mask=context_mask,
                                                         cascade_cond=cascade_cond, cascade_mask=cascade_mask, cascade_time=cascade_time)
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.dataloader = cascade_dataset.get_dataloader(args, self.dataset['train'], self.bart_model.config, self.text_tokenizer, self.syntax_tokenizer, self.max_seq_len,
                                                         context_max_seq_len=self.context_max_seq_len, cascade=self.cascade)
    ...
    @torch.no_grad()
    def sample_text2syn(self, num_samples=None, class_id=None, split='val', seed=77, cls_free_guidance=1.0,):
        ...
        for batch in dataloader:
            ...
            context = self.text_encoder(input_ids = data['context_input_ids'], attention_mask = data['context_attention_mask']).last_hidden_state.float()
            context_mask = data['context_attention_mask'].bool()
          
            if self.class_conditional:
                class_id = data['label']

            z_0, mask = self.ema.ema_model.sample(batch_size=context.shape[0], length=self.length_categorical.sample((context.shape[0],)), class_id=class_id, context=context, context_mask=context_mask, cls_free_guidance=cls_free_guidance)
            ...
            gen_syntax = [self.syntax_tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=True).strip() for g in sample_ids]
            print(gen_syntax) # debug

            pred_syntax.extend(gen_syntax)
            if self.class_conditional:
                ref_label.extend([tensor.item() for tensor in class_id])
            ref_syntax.extend([self.syntax_tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=True).strip() for g in data['input_ids']])
            ...
            if len(pred_syntax) >= num_samples:
                break

        assert len(pred_syntax) == len(ref_syntax)
        assert len(pred_syntax) >= num_samples
        pred_syntax = pred_syntax[:num_samples]
        ref_syntax = ref_syntax[:num_samples]
        ...
        file_utils.save_samples(path, pred_syntax, ref_syntax, cond_text=cond_texts, class_id=ref_label)
          
        # Log samples
        # source | reference | pred
        if self.class_conditional:
            columns = ['class', 'context', 'reference', 'prediction']
        else:
            columns = ['context', 'reference', 'prediction']
        data = []
        for i in range(len(pred_syntax)):
            if self.class_conditional:
                row = [ref_label[i], cond_texts[i], ref_syntax[i], pred_syntax[i]]
            else:
                row = [cond_texts[i], ref_syntax[i], pred_syntax[i]]
            data.append(row)
        table = wandb.Table(columns=columns, data=data)
        accelerator.log({f"text2syn/{prefix}{split}_samples": table}, self.step)

        # Compute metrics
        metrics = {}
        metrics[f"text2syn/{prefix}unique_wordcount"] = evaluation.compute_wordcount(pred_syntax)
        ngram_metrics = evaluation.compute_diversity(pred_syntax)
        for k, v in ngram_metrics.items():
            metrics[f"text2syn/{prefix}{k}"] = v
    ...
    def sample_syn2text(self, num_samples=None, class_id=None, split='val', seed=77, cls_free_guidance=1.0,):
        ...
        for batch in dataloader:
            ...
            z_0, mask = self.ema.ema_model.sample(batch_size=cascade_syntax.shape[0], length=length, class_id=class_id, context=context, context_mask=context_mask,
        ...
        assert len(pred_texts) == len(ref_texts) == len(cond_syntax) == len(cond_texts)
        ...
        milestone = self.step // self.save_and_sample_every
        path = os.path.join(self.results_folder, f'{"eval-" if self.args.eval else ""}{f"cascade-" if self.args.cascade_gen else ""}sample-{split}-{milestone}.txt')
        file_utils.save_samples(path, pred_texts, ref_texts, cond_text=cond_texts, cond_syn=cond_syntax, class_id=ref_label)
          
        # Log samples
        # source | reference | pred
        if self.class_conditional:
            columns = ['class', 'context', 'cond_syntax', 'reference', 'prediction']
        else:
            columns = ['context', 'cond_syntax', 'reference', 'prediction']
        data = []
        for i in range(len(pred_texts)):
            if self.class_conditional:
                row = [ref_label[i], cond_texts[i], cond_syntax[i], ref_texts[i], pred_texts[i]]
            else:
                row = [cond_texts[i], cond_syntax[i], ref_texts[i], pred_texts[i]]
            data.append(row)
        table = wandb.Table(columns=columns, data=data)
        ...
        for k, v in ngram_metrics.items():
            metrics[f"syn2text/{prefix}{k}"] = v
        metrics[f"syn2text/{prefix}bertscore"] = evaluation.compute_bertscore(pred_texts, ref_texts)
        metrics[f'syn2text/{prefix}bleu'] = evaluation.compute_bleu(pred_texts, ref_texts)
    ...
    def sample_cascade(self, num_samples=None, class_id=None, test=False, seed=77, cls_free_guidance=1.0):
        ...
        if test:
            if exists(class_id):
                ...
                dataloader = cascade_dataset.get_class_dataloader(self.args, class_subset, self.bart_model.config, self.text_tokenizer, self.syntax_tokenizer, self.max_seq_len,
                                                         context_max_seq_len=self.context_max_seq_len, batch_size=batch_size)
            else:
                dataloader = self.test_dataloader
        else:
            dataloader = self.val_dataloader
        ...
        for batch in dataloader:
            ...
            cond_texts.extend([self.text_tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=True).strip() for g in data['context_input_ids']])
            ref_texts.extend([self.text_tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=True).strip() for g in data['input_ids']])

            # Step 1: generate syntax
            syn_z_0, syn_mask = self.cascade_syntax_ema.ema_model.sample(batch_size=batch_size, length=length, class_id=gen_class_id, context=context, context_mask=context_mask, cls_free_guidance=cls_free_guidance)
            ...
            gen_syntax = [self.syntax_tokenizer.decode(g, skip_special_tokens=False, clean_up_tokenization_spaces=True) for g in sample_ids]
            print(gen_syntax) # debug
            pred_syntax.extend(gen_syntax)
            ...
            text_z_0, mask = self.ema.ema_model.sample(batch_size=batch_size, length=length, class_id=gen_class_id,  context=context, context_mask=context_mask,
                                                        cascade_cond=cascade_syntax, cascade_mask=cascade_syntax_mask, cascade_time=cascade_time, cls_free_guidance=cls_free_guidance)
        ...
        file_utils.save_samples(path, pred_texts, ref_texts, cond_text=cond_texts, cond_syn=pred_syntax, class_id=ref_labels)
        # Log samples
        # source | reference | pred
        data = []
        if self.class_conditional:
            columns = ['class', 'context', 'syntax', 'reference', 'prediction']
        else: 
            columns = ['context', 'syntax', 'reference', 'prediction']
        for i in range(len(pred_texts)):
            if self.class_conditional:
                row = [ref_labels[i], cond_texts[i], pred_syntax[i], ref_texts[i], pred_texts[i]]
            else:
                row = [cond_texts[i], pred_syntax[i], ref_texts[i], pred_texts[i]]
            data.append(row)
        table = wandb.Table(columns=columns, data=data)
        ...
        for k, v in ngram_metrics.items():
            metrics[f"cascade/{prefix}{k}"] = v
        metrics[f"cascade/{prefix}bertscore"] = evaluation.compute_bertscore(pred_texts, ref_texts)
        metrics[f'cascade/{prefix}bleu'] = evaluation.compute_bleu(pred_texts, ref_texts)
        # Only evaluate MAUVE if generations are reasonable to speed up validation early on
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    loss = self.diffusion(z_0, mask, class_id=(data['label'] if self.class_conditional else None), context=context, context_mask=context_mask,
                                           cascade_cond=cascade_cond, cascade_mask=cascade_mask, cascade_time=cascade_time)
                    ...
                    if self.step % 50 == 0:
                        ...
                        with torch.no_grad():
                            ...
                            for _ in range(self.gradient_accumulate_every):
                                ...
                                loss = self.ema.ema_model(z_0, mask, class_id=(data['label'] if self.class_conditional else None), context=context, context_mask=context_mask,
                                                       cascade_cond=cascade_cond, cascade_mask=cascade_mask, cascade_time=cascade_time)
                                loss = loss / self.gradient_accumulate_every
                        ...
                        if self.class_conditional:
                            for class_id in range(self.diffusion.diffusion_model.num_classes):
                                if self.cascade:
                                    ...
                                    self.sample_text2syn(num_samples=100, class_id=class_id)
                        else:
                            if self.cascade:
                                self.sample_syn2text(num_samples=100)
                            else: 
                                self.sample_text2syn(num_samples=100)
