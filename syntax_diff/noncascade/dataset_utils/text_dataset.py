...
def get_dataloader(args, dataset, model_config, text_tokenizer, syntax_tokenizer, max_seq_len, seq2seq=None, context_max_seq_len=None, shuffle=False):
    ...
    if 'bart' in args.enc_dec_model:
        ...
        collate_fn=DataCollatorForFlanLM(text_tokenizer)
...
def get_class_dataloader(args, dataset, model_config, text_tokenizer, syntax_tokenizer, max_seq_len, batch_size, seq2seq=None, context_max_seq_len=None, shuffle=False):
    def tokenization(example):
        syntax = example['syntax']
        target = example['text']
        syntax_inputs = syntax_tokenizer(syntax, padding="max_length", truncation=True, max_length=max_seq_len//2)
